package com.arch.componentlib.router;
...
public class Router {
    ...
    //注册的组件的集合
    private static HashMap<String, IRouterApplication> components = new HashMap<>();
    ...
    public static void registerComponent(@Nullable String classname) {
        ...
        try {
            ...
            IRouterApplication applicationRouter = (IRouterApplication) clazz.newInstance();
            applicationRouter.onCreate();
            components.put(classname, applicationRouter);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
